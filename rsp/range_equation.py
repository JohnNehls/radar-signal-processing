...
def snr_rangeEquation(Pt, Gt, Gr, sigma, wavelength, R, B, F, L, T, time_bandwidth_prod):
    """Single-pulse SNR"""
    return (
        snr_rangeEquation_uncoded(Pt, Gt, Gr, sigma, wavelength, R, B, F, L, T)
        * time_bandwidth_prod
    )


def snr_rangeEquation_CP(Pt, Gt, Gr, sigma, wavelength, R, B, F, L, T, n_p, time_bandwidth_prod):
    """ "SNR of range equation with coherent processing (CP)"""
    singlePulse_snr = snr_rangeEquation(
...
def minTargetDetectionRange_BPSK_pulses(
...
def minTargetDetectionRange_dutyFactor_pulses(
